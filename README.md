...
用 C++ 编写，**判题交给洛谷**：题目页只有「Monaco 编辑器 + 提交到洛谷」，
没有本地运行 / 提交 / 测试用例面板，因此打开 Pro 题目页**不需要下载 90MB 的编译器**。
...
重点是**算法思路、复杂度分析与边界处理**（而不是基础语法）。每道题都标注了**洛谷对应题目**，
写完代码点「提交到洛谷」即可拿到洛谷官方的评测结果——**洛谷返回 AC 才算通过本题，并解锁下一题**。

### 洛谷远程提交（`/luogu`）

绑定一次洛谷账号后，题目页会出现「提交到洛谷」面板：洛谷题号**已经按题库标注预填好**（也可以自己改，改完会记住）、
选洛谷评测语言（默认按靶场自动选：C++20 / Python 3 / Java 8 = 洛谷语言 27 / 7 / 8）、一键提交，
**实时轮询洛谷的评测结果**（AC / WA / TLE / MLE / RE / CE……，含编译错误原文），
**评测结束后自动刷新该题的提交记录列表**，也可以随时手动刷新——就是 vjudge 那种远程提交体验。
**四个靶场的题目页都有这个面板**。

四个靶场的题目列表也都多了一列「洛谷」，点题号直接去洛谷；没有标注的题显示 `—`。
...
**Pro 靶场的 42 道题对应到洛谷的精确题号**（41 道有题号，每道都用 `npm run validate:luogu` 对着洛谷真实页面核对过，
例如 p1-5 下一个更大元素 → `P5788 【模板】单调栈`），题目页上这一栏写的是「洛谷对应题目」。

**C++ / Python / Java 这三个基础靶场也各自标注了洛谷题目**（126 道题里 122 道有题号），
映射表在 `src/data/luogu-codes.ts`，一次人工核对、三轨共用。这三个靶场的题是本站自撰的，
所以那一栏的文案是「洛谷**同类型练习**」而不是"原题"——例如：

| 本站题目 | 洛谷同类型练习 |
| --- | --- |
| 你好，世界 / 你好，Python / 你好，Java | `B2002 Hello,World!` |
| 两数之和 | `P1001 A+B Problem` |
| 温度换算 | `B2013 温度表达转化` |
| 简易计算器 | `B2052 简单计算器` |
| 闰年判断 | `B2150 判断闰年` |
| 九九乘法表 | `P8723 [蓝桥杯 2020 省 AB3] 乘法表` |
| 回文串判断 | `B2124 判断字符串是否为回文` |
| 第一个只出现一次的字符 | `B2110 找第一个只出现一次的字符` |
| 按成绩排名（结构体排序） | `P1093 [NOIP 2007 普及组] 奖学金` |

洛谷确实没有的练习就**留空**（例如"交换两个数"和 Pro 的"反转链表"），不硬塞一道不相关的题。

本站仍然**不抓取、不缓存洛谷题面**：题目描述、测试用例与参考题解都是本站自撰，洛谷只作为"在线评测与额外练习"使用。

四个靶场是**同一套页面结构**，用顶栏右上角的 `C++ / Python / Java / Pro 靶场` 按钮切换（各自的进度互相独立）：

```
首页（阶段卡片 + 进度统计 + 搜索筛选）
  └─ 阶段页（阶段简介 + 题目列表：状态 / 难度 / 知识点 / 洛谷题号）
       ├─ C++ / Python / Java 题目页（左侧题目描述 + 提示 + 题解 ｜ 右侧编辑器 + 标准输入 / 运行结果 + 测试用例）
       └─ Pro 题目页（左侧题目描述 + 提示 + 题解 ｜ 右侧编辑器 + 提交到洛谷，判题在洛谷）
            └─ 进度页（统计 + 各阶段完成情况）   ·   指南页（零基础语法速查）
```

Java 靶场同样支持：顺序解锁与开发者模式、`写到哪？`（光标跳到 `// TODO`）、
中文标点与 javac 报错的中文自查提示、每题的本关新知识与常见错误。
C++ 靶场还多一个**开发者面板**（编译参数 / 工具链来源 / 耗时 / 原始诊断 / 复制题目 JSON）；
Pro 靶场因为不本地编译，开发者模式下提供「填入参考题解」与「复制题目 JSON」两个按钮。
...
│  ├─ luogu-codes.ts     C++ / Python / Java 三轨共用的「洛谷同类型练习」映射表
│  ├─ guide.md           C++ 新手指南（第零课）
...
| `npm run validate:luogu` | 校验四个靶场的洛谷题号：逐个访问洛谷题目页，确认存在且标题能取到（**真实网络请求**，只读）；可用 `--track=cpp` / `--problem=p1-1` 缩小范围 |
...
- 洛谷题号：`npm run validate:luogu` → **四个靶场 88 个不同题号全部存在且标题核对一致**
  （逐题访问洛谷题目页；Pro 41/42、C++ 40/42、Python 41/42、Java 41/42 标注了题号，
  留空的 5 道是「交换两个数」×3 与「反转链表」「Pro 反转链表」——洛谷确实没有对应练习）
...
- Pro 靶场改为「判题在洛谷」后的验收（`scripts/e2e-pro.mjs`，41 项）：题目页确实
  **没有本地运行 / 提交 / 测试用例面板 / 编译器提示**，用真实桥接脚本提交一次拿到 AC 后
  本题计入通过、下一题解锁、第三题仍锁着、进度页显示「已通过 1」；
  并且断言整个 Pro 流程**没有任何 `/toolchain/` 或 jsDelivr 请求**（不再下载 90MB 编译器）
...
- 基础靶场的洛谷题号标注的是**同类型练习**（本站题目为自撰，不是洛谷原题的移植），
  Pro 靶场那 41 道才是按同一种算法对应的题目。
