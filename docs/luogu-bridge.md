...
| 路径 | 是什么 | 安装成本 | 适用 |
| --- | --- | --- | --- |
| **A. 桥接脚本** | 一个油猴脚本（`public/oiworld-luogu.user.js`），由扩展用 `GM_xmlhttpRequest` 代网页发请求 | 装一次 Tampermonkey / Violentmonkey | 经常交题：在 OIworld 里点按钮即可提交、看结果、拉记录 |
...
Pro 靶场的 42 道题里，**41 道**已经对应到洛谷的精确题号（写在题目数据的 `luogu_code` 字段里），
题目页与题目列表都会显示并可点击；剩下 1 道「反转链表」是因为**洛谷确实没有对应的链表反转题**，
只保留关键词搜索链接——宁可空着，也不塞一个不相关的题号进去。
...
npm run validate:luogu                        # 门禁：逐个访问题库里写的每个题号，标题必须能取到
```

`npm run validate:luogu` 是内容门禁的一部分：题号写错会让人把代码交到别的题上，
肉眼很难发现，所以交给脚本对真实洛谷校验。

## 8. 可以继续做的

1. 把提交成功后自动刷新提交记录（现在需要点一次「刷新提交记录」）；
2. 为 C++ / Python / Java 三个基础靶场的题目也标注洛谷题号（面板本身已经是通用的，
   现在在这三个靶场里是手动填题号，填过会记住）；
3. 把洛谷的评测详情（每个测试点、编译信息）做成更细的展示。
