...
interface Props {
  entries: JavaProblemEntry[];
  showStage?: boolean;
}

interface Row extends JavaProblemEntry {
  key: string;
}

export function JavaProblemTable({ entries, showStage = false }: Props) {
  ...
  const open = (id: string, unlocked: boolean) => {
    if (unlocked) navigate(`/java/problem/${id}`);
  };
  ...
  const columns: ColumnsType<Row> = [
    {
      ...
      render: (_, row) => (
        <StatusTag
          status={getJavaProblemStatus(row.problem.id, completed, attempted)}
        />
      ),
    },
    {
      title: '题目',
      render: (_, row) => {
        const unlocked = isJavaProblemUnlocked(
          row.problem.id,
          completed,
          developerMode,
        );
    ...
    },
    {
      ...
      width: 92,
      render: (_, row) => (
        <Tag color={JAVA_DIFFICULTY_COLOR[row.problem.difficulty]}>
