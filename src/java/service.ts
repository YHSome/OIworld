...
class JavaService {
  ...
  private async compile(code: string) {
    ...
    return { ok: exit === 0, diagnostics: `${this.output.stdout}${this.output.stderr}`, durationMs: 0 };
  }
  private async run(stdin: string) {
    ...
    return { ok: exit === 0, stdout: this.output.stdout, stderr: this.output.stderr, durationMs: 0 };
  }
  async runOnce(code: string, stdin: string): Promise<RunOutcome> {
    const task = async () => { const compiled = await this.compile(code); if (!compiled.ok) return { status: 'compile-error' as const, diagnostics: compiled.diagnostics, stdout: '', stderr: '', exitCode: null, compileMs: 0, runMs: 0 }; const run = await this.run(stdin); return { status: run.ok ? 'passed' as const : 'runtime-error' as const, diagnostics: '', stdout: run.stdout, stderr: run.stderr, exitCode: run.ok ? 0 : null, compileMs: 0, runMs: run.durationMs, message: run.ok ? undefined : 'Java 程序没有正常结束。' }; };
