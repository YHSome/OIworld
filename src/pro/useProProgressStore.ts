...
import { PRO_PROBLEM_ENTRIES } from './data';

const ids = new Set(PRO_PROBLEM_ENTRIES.map((entry) => entry.problem.id));

interface ProProgressState {
...
/** Pro 靶场（数据结构与进阶算法）的进度（与 C++ / Python / Java 各自独立保存） */
export const useProProgressStore = create<ProProgressState>()(
  persist(
    (set) => ({
      completedProblems: [],
      attemptedProblems: [],
