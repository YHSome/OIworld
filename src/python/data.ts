  ...
  stage(1, '第一阶段：开口说话', '输出、变量与输入', '从 `print()` 开始，学会把数据读进来、算出来、输出去。', [
    {
      ...
      description: `## 欢迎来到第一关：让程序开口说话

编程并不是让电脑“理解你的想法”，而是把每一步写成它认识的指令。这一关只做一件很小但很重要的事：让电脑在屏幕上显示一句话。
...
英文大小写、逗号后的空格、感叹号都属于答案的一部分。不要额外输出“我的答案是”或“Hello Python”。
...
| 部分 | 怎么理解 |
| --- | --- |
| \`print\` | 英文“打印”。在 Python 中它负责把内容显示到输出区。必须全小写。 |
| \`(\` 与 \`)\` | 圆括号像一个小盒子，把要交给 print 的内容装进去。左右必须成对。 |
| \`"Hello, Python!"\` | 双引号里的内容叫**字符串**，也就是原样显示的文字。 |
| 这一行的末尾 | Python 用换行表示一句结束，**不用像 C++ 那样写分号**。 |

### 为什么文字必须有引号

有引号的 \`"Hello"\` 是一段文字；没有引号的 \`Hello\` 会被 Python 当作一个变量名。你还没创建这个名字，程序就会报 \`NameError\`。

### 英文标点很重要

请在英文输入法状态下输入圆括号和双引号。中文的 \`（ ）\`、\`“ ”\` 看起来相似，但 Python 不认识。
...
以 \`#\` 开头的是**注释**，它只给人看，Python 会跳过它。\`TODO\` 可以理解为“待完成”：把这一整行替换为输出语句即可。

## 按这四步完成第一题

1. 点击编辑器，在 \`# TODO\` 那一行输入或粘贴输出语句。
2. 点击“运行”。运行只会执行一次，并把结果展示出来。
3. 确认输出与题目完全一致。
4. 点击“提交”。提交会使用题目的测试用例评测，全部通过后才会解锁第二关。

## 常见错误

| 写法 | 会发生什么 | 修正 |
| --- | --- | --- |
| \`Print("Hello, Python!")\` | Python 区分大小写，不认识大写 Print。 | 必须写小写 \`print\`。 |
| \`print(Hello, Python!)\` | 文字没有引号，逗号也会造成语法问题。 | 整段文字放进一对英文双引号。 |
| \`print("Hello, Python!\`) | 少了右引号，字符串没有结束。 | 左右引号都要有。 |
| \`print("Hello, Python!")；\` | 中文分号会报错。 | 删除分号，或使用英文符号。 |`,
...
| 使用中文括号或中文引号 | Python 不认识全角标点。 | 切换英文输入法，使用 \`()\`、\`""\`。 |`,
    ...
    },
    {
      ...
      description: `## 题目背景：变量是贴了名字的数据盒子

上一题中你把输入的两个数叫做 \`a\`、\`b\`。这道题仍然读两个数，但我们给它们起更有意义的名字：\`length\`（长）和 \`width\`（宽）。

变量名不是魔法单词，而是你给数据起的**标签**。名字取得清楚，过几天再看代码也能知道每个数代表什么。
...
计算 \`length * width\`，也就是 \`4 * 6\`，结果是 \`24\`，所以输出：
...
和第二关的 \`a, b = map(int, input().split())\` 是**同一种写法**，只是变量名变了。

1. \`input()\` 读到整行文字，例如 \`"4 6"\`。
2. \`.split()\` 按空格切成 \`"4"\`、\`"6"\`。
3. \`map(int, ...)\` 把这两段文字变成整数 \`4\`、\`6\`。
4. 第一个整数存进 \`length\`，第二个存进 \`width\`。

> 变量名可以很长，也可以包含下划线，例如 \`total_score\`。但不能有空格，且要前后一致。\`width\` 和 \`Width\` 是两个不同的名字。
...
把这个结果交给 \`print\` 就能输出。你只要在 TODO 处完成这一件事。

## 动手步骤

1. 保留第一行读入代码。
2. 在 TODO 处写一行 \`print(...)\`，括号里放“长乘宽”的表达式。
3. 用标准输入框填入 \`4 6\`，点击“运行”，应看到 \`24\`。
4. 再试试 \`1 99\`；如果输出 \`99\`，说明没有把数字写死。
5. 点击“提交”让测试用例检查。
...
| 容易写错的内容 | 问题 | 正确做法 |
| --- | --- | --- |
| \`length × width\` | 中文乘号 \`×\` 不是 Python 符号。 | 使用英文星号 \`*\`。 |
| \`print(length + width)\` | 这是周长的一部分，不是面积。 | 使用乘法。 |
| \`print("length * width")\` | 引号会让它变成普通文字。 | 表达式不能加引号。 |
